...
const UpdateProduct = ({ productId }) => {
  ...
  const handleSubmit = async (e, productId) => {
    ...
    if (price)
      productInfo['price'] =
        price.indexOf(',') !== -1 ? Number(price.replace(/,/, '.')) : +price;
    if (location) productInfo['location'] = location;
    if (description) productInfo['description'] = description;

    const ret = await fetch(
      `https://api.retropia-market.com/catalogue/${productId}/update`,
      {
        method: 'PATCH',
    ...
    <div className="update-product">
      ...
      {showUpdate && (
        <div className="update-modal">
          <div className="modal-bg" onClick={() => setShowUpdate(false)}>
            <motion.form
              className="modal-fg"
              variants={item}
              animate="visible"
            ...
            >
              ...
              <div className="update-inputs">
                ...
                <label>
                  ...
                  <SelectSaleState setProductStatus={setStatus} />
                </label>
                <label>
                  <FormattedMessage id="sale.productLoc" />
                  <LocationSelector setProductLocation={setLocation} />
                </label>
