...
const GiveFavComponent = ({ data }) => {
  ...
  const apiFavURL = `https://api.retropia-market.com/${user.userData.id}/getFavourites/`;
  ...
  const handleFav = async () => {
    if (!fav) {
      const ret = await fetch(
        `https://api.retropia-market.com/${data.id}/addFavourite/${user.userData.id}`,
        {
          method: 'POST',
          headers: {
      ...
      const ret = await fetch(
        `https://api.retropia-market.com/${data.id}/removeFavourite/${user.userData.id}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: 'Bearer ' + user.token,
