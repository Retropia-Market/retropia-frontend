...
function ChatRoom () {
  ...
  useFetchChat(`http://localhost:8080/chats/${user.userData.id}/get-messages/${id}`, 'fetch/messages')
  const [message, setMessage] = useState('')

  const handleSubmit = e => {
    e.preventDefault()
    setMessage('')
    fetch('http://localhost:8080/chats/' + user.userData.id + '/send-message/' + id, {
      method: 'POST',
      headers: {
  ...
  return <div className="chat-room">
    ...
    <div className="messages">
        {messages.reverse().map(m =>
          <div className={'message ' + (m.dst_id === parseInt(id) ? 'own' : 'remote')} key={m.id}>
            {m.message}
          </div>
        )}
    </div>
