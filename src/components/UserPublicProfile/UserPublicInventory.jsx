...
const UserPublicInventory = ({ uid }) => {
  const apiURL = `https://api.retropia-market.com/users/${uid}/catalogue`;
  ...
  return (
    <div className="user-public-inventory">
      <div className="inventory">
        {results &&
          results.some((p) => p.sale_status.toLowerCase() === 'en venta') && (
            <>
              ...
              <div className="inventory-products">
                {results &&
                  results
                    .filter((p) => p.sale_status.toLowerCase() === 'en venta')
                    .map((product) => (
      ...
      <div className="inventory">
        {results &&
          results.some((p) => p.sale_status.toLowerCase() === 'vendido') && (
            <>
              <h1 className="main-title title-inventory">
              ...
              <div className="inventory-products sold">
                {results &&
                  results
                    .filter((p) => p.sale_status.toLowerCase() === 'vendido')
                    .map((product, i) => (
                      <ProductCard data={product} key={product.id} />
                    ))}
              </div>
